...
| Databricks Free Edition / PySpark | Free/OSS | **No** | Rs 0 | Layer 4's unified-dataset build uses plain pandas directly in `metrics_collector.py`/`anomaly_detection.py` instead - at this data volume (single-host demo, hundreds not millions of rows), a distributed/batch engine added setup and account-provisioning overhead with no processing benefit. Functionally equivalent unified dataset is still produced. |
| Azure Monitor Free Tier | Free tier | **No** | Rs 0 | Proposed as a second cloud-based metric sink alongside Prometheus (Layer 2); not implemented - Prometheus (local) + Jaeger (traces) fully cover the demo's telemetry needs without adding a cloud account dependency. No budget impact since it was never provisioned. |
...
| Layer 4 processing engine | Databricks Free Edition / PySpark | Plain pandas | At single-host demo data volumes, a distributed batch engine added account/cluster setup overhead with no processing benefit; pandas produces the same unified dataset schema. |
...
| Gemini/Ollama live generation in the submitted run | No `GOOGLE_API_KEY` provisioned; Ollama server (v0.33.3) is running and reachable on this machine, but has zero models pulled (`ollama list` is empty) - not an unreachability problem, just needs one `ollama pull <model>` | N | Provision a Gemini API key via `.env` (code path already implemented and tested); run `ollama pull llama3.2` (or the RFP's named Llama 4 Scout, if available in the Ollama library by demo time) to activate the offline fallback for real |
