...
| Google Gemini 2.5 Flash API | Free tier | Partial | Rs 0 | Code path implemented and tested (`ai_annotation.py`); no API key was provisioned in time for this submission, so Ollama (below) or the deterministic fallback generated the shipped report instead. Swapping in a `GOOGLE_API_KEY` env var activates it with no code change. |
